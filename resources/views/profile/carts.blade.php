<x-app-layout>
    <div>
                ...
                @foreach($products as $product)
                    ...
                    <article class="card bg-base-100 shadow-sm hover:shadow-xl border border-base-200 transition-all duration-300 group rounded-xl overflow-hidden">
                       ...
                       <div class="flex justify-center">
                        {{-- Image + Like button --}}
                        <figure class="relative h-24 w-50 overflow-hidden bg-base-200">
                            <a href="{{ route('products.show', $product->slug) }}">
                                <img src="{{ $image }}"
                                        alt="{{ $product->name }}"
                                        class="h-24 w-50 object-cover transition-transform duration-500 group-hover:scale-105"
                                        loading="lazy" />
                            </a>
                            @if($product->stock <= 0)
        ...
        @endif
         <a href="#"  class="dock bg-neutral text-neutral-content">
            ${{number_format($tootle)}}
         </a>
    </div>
</x-app-layout>
