<x-app-layout>
    <x-categories :categories="$categories" :categorySlug="$category->slug" />
        ...
        <div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 bg-base-100 p-4 rounded-xl border border-base-200 shadow-sm">
                <div class="text-sm font-semibold text-base-content/70 text-right">
                    ...
                    @if($search)
                        لـ "<span class="text-primary font-bold">{{ $search }}</span>"
                    @endif
                </div>

                <form method="GET" action="{{ route('category.show',$category->slug) }}" class="flex items-center gap-3">
                    ...
                    <select name="sort"
                            class="select select-bordered select-sm rounded-lg text-xs font-bold"
                            onchange="this.form.submit()">
                        <option value="created_at" {{ $sortBy === 'created_at' ? 'selected' : '' }}>الأحدث إضافة</option>
                        <option value="price_asc"  {{ $sortBy === 'price_asc'  ? 'selected' : '' }}>السعر: من الأقل</option>
                        <option value="price_desc" {{ $sortBy === 'price_desc' ? 'selected' : '' }}>السعر: من الأعلى</option>
                        <option value="name"       {{ $sortBy === 'name'       ? 'selected' : '' }}>الاسم أبجدياً</option>
                    </select>
                </form>
            </div>
            {{-- Products grid --}}
            @if($products->isEmpty())
                <div class="bg-base-100 rounded-xl border border-base-200 shadow-sm p-12 text-center flex flex-col items-center">
                    ...
                    <p class="text-sm text-base-content/60 mb-6">جرب تغيير الفلاتر أو كلمات البحث.</p>
